...
/** Ceiling on pinned lines in the digest; pinned entries are never budget-dropped. */
 ...
 *
 * The contract that makes a standing user rule reliable: a `pinned` entry is
 * NEVER dropped to fit the budget, and `global` (cross-project) pinned entries
 * come first, because a busy project must not be able to starve a rule that
 * applies everywhere. `digestChars` therefore governs only the unpinned
 * "recent" tail; the separate `maxPinned` ceiling is what keeps pinned output
 * bounded when someone pins dozens of entries.
 */
function digest() {
  ...
  // Pinned first, global scope ahead of the project scope, never budget-dropped.
  const pinnedOrder = keys.slice().sort((a, b) => Number(b === GLOBAL_SCOPE) - Number(a === GLOBAL_SCOPE))
  for (const key of pinnedOrder) {
    for (const entry of entriesOf(key).filter((item) => item.pinned === true).sort(byUpdated)) {
      if (pinnedShown >= STATE.maxPinned) {
        pinnedOmitted += 1
        continue
      }
      const line = entryLine(entry, true, multi, key)
      lines.push(line)
      used += line.length + 1
      pinnedShown += 1
    }
  ...
  // Then the unpinned tail, project scope first, sharing what is left.
  for (const key of keys) {
    for (const entry of entriesOf(key).filter((item) => item.pinned !== true).sort(byUpdated)) {
      const line = entryLine(entry, false, multi, key)
      if (used + line.length + 1 > STATE.digestChars) {
        unpinnedHidden += 1
        continue
      }
      lines.push(line)
  ...
  let out = HEAD + '\n' + lines.join('\n')
  const notes = []
  if (pinnedOmitted > 0) notes.push('+' + pinnedOmitted + ' pinned over maxPinned=' + STATE.maxPinned)
  if (unpinnedHidden > 0) notes.push('+' + unpinnedHidden + ' more; memory action=list')
...
export function apply(ctx, config) {
  ...
  STATE.maxPinned = Number.isFinite(options.maxPinned) ? Math.max(1, options.maxPinned) : DEFAULT_MAX_PINNED
