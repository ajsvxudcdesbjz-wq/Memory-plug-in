...
const HEAD =
  ...
  'Memory recorded by other projects or sessions is NOT injected: fetch it deliberately with memory action=scopes or action=search scope=all.'
...
/** Read one stored document, or '' when it is absent. */
function readDocFile(name) {
  try {
    return readFileSync(join(sessionsDir(), name), 'utf8')
  } catch {
    return ''
  }
}

/** Write one stored document. */
function writeDocFile(name, text) {
  mkdirSync(sessionsDir(), { recursive: true })
  writeFileSync(join(sessionsDir(), name), text, 'utf8')
}
